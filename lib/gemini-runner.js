...
async function fetchGemini(body, maxRetries = 4) {
  ...
  for (let attempt = 1; attempt <= maxRetries; attempt++) {
    ...
    if (res.status === 429) {
      // Retry-After 헤더 확인, 없으면 지수 백오프 (60s, 120s, 180s)
      const retryAfterSec = parseInt(res.headers.get("retry-after") ?? "0", 10);
      const waitMs = retryAfterSec > 0 ? retryAfterSec * 1000 : 60_000 * attempt;
      if (attempt < maxRetries) {
        console.warn(`[gemini] 429 속도 제한 — ${waitMs / 1000}초 대기 후 재시도 (${attempt}/${maxRetries - 1})`);
        await new Promise((r) => setTimeout(r, waitMs));
        continue;
      }
      throw new Error(`[gemini] HTTP 429: 속도 제한 — ${maxRetries}회 재시도 후 실패`);
    }

    if (res.status === 503 || res.status === 529) {
      // 서버 과부하 — 내부에서 재시도 (30s, 60s, 120s)
      const waitMs = 30_000 * attempt;
      if (attempt < maxRetries) {
        console.warn(`[gemini] ${res.status} 서버 과부하 — ${waitMs / 1000}초 대기 후 재시도 (${attempt}/${maxRetries - 1})`);
        await new Promise((r) => setTimeout(r, waitMs));
        continue;
      }
      throw new Error(`[gemini] HTTP ${res.status}: 서버 과부하 — ${maxRetries}회 재시도 후 실패`);
    }
